<#macro table cols normalOperate=[] batchOperate=[] wholeOptName="" wholeOptAction="" keepParams=[] gotoPageAction="Com_list"
  actionSuffix=".jhtml" keyId="id" batchId="ids" wholeId="wids" value=pagination isPagination=true rowIndex=true operateCol="操作" checkRight="true"
  width="100%">
...
<tr onmouseover="Pn.LTable.lineOver(this);" onmouseout="Pn.LTable.lineOut(this);" onclick="Pn.LTable.lineSelect(this);">
	...
	<#if rowIndex>
	<td align="center">${row_index+1}</td>
	</#if>
	<#list cols as col>
	<td<#if col['width']??> width="${col['width']}"</#if><#if col['align']??> align="${col['align']}"</#if><#if col['title']??> title="${(col['title']?eval)!}"</#if>><#rt/>
